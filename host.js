...
// from this page (persisted to localStorage) instead of needing a code
// change every time a new host comes on board.
...
let customHosts = loadCustomHosts();
let hiddenHostKeys = loadHiddenHostKeys();
...
function deleteHost(key) {
  ...
  const customIndex = customHosts.findIndex(c => c.key === key);
  if (customIndex !== -1) {
    customHosts.splice(customIndex, 1);
    saveCustomHosts(customHosts);
  } else {
    hiddenHostKeys.push(key);
    saveHiddenHostKeys(hiddenHostKeys);
  }
  renderHosts();
}
...
if (hostForm) {
  hostForm.addEventListener('submit', (e) => {
    ...
    customHosts.push({
      key: slugify(name),
      name,
      icon: NEW_HOST_ICONS[customHosts.length % NEW_HOST_ICONS.length],
      keywords,
    });
    saveCustomHosts(customHosts);
